...
export default function Skills() {
  ...
  const categories_list = [
    { id: 'all', label: 'All Skills', count: skills?.length || 0 },
    ...
    { id: 'collaboration', label: 'Collaboration', count: categories?.collaboration?.total || 0 },
  ]
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
          ...
          <div className="p-4 bg-green-50 rounded-lg">
            <p className="text-2xl font-bold text-green-700">
              {skills?.filter((s: any) => s.enabled).length || 0}
            </p>
          ...
          <div className="p-4 bg-red-50 rounded-lg">
            <p className="text-2xl font-bold text-red-700">
              {skills?.reduce((acc: number, s: any) => acc + (s.metrics?.total_executions || 0), 0) || 0}
            </p>
